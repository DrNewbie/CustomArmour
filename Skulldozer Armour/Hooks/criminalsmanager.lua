Hooks:PostHook(CriminalsManager, "add_character", "SkulldozerArmour_add_character", function(self, name, unit, peer_id, ai, ai_loadout)
	if alive(unit) then
		local peer = managers.network:session():peer(peer_id)
		if peer then
			if tostring(peer:armor_id(true)) == "level_12" then
